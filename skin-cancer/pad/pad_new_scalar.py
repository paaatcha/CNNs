...
paths_train, labels_train, scalar_feat_train, dict_labels_train = get_path_and_labels_from_folders (path_train, 'txt', one_hot=False)
train_data, train_dataset = get_dataset_tf (paths_train, labels_train, True, params_load, scalar_feat=scalar_feat_train)

params_load['shuffle'] = False
#params_load['batch_size'] = 10

# Loading the validation dataset and creating the dataser TF iterator
paths_val, labels_val, scalar_feat_val, dict_labels_val = get_path_and_labels_from_folders (path_val, 'txt', one_hot=False)
val_data, val_dataset = get_dataset_tf (paths_val, labels_val, False, params_load, scalar_feat=scalar_feat_val)

# Loading the test dataset and creating the dataser TF iterator
paths_test, labels_test, scalar_feat_test, dict_labels_test = get_path_and_labels_from_folders (path_test, 'txt', one_hot=False)
test_data, test_dataset = get_dataset_tf (paths_test, labels_test, False, params_load, scalar_feat=scalar_feat_test)   
